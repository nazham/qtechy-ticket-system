...
function AdminCard({
  ...
  return (
    <div
      ...
      style={{
        borderColor: borderAccent ? undefined : 'var(--color-neutral-border)',
      }}
    >
      {/* Top stripe */}
      {borderAccent && (
        <div
          className={`h-1 w-full ${borderAccent.replace('border-', 'bg-').split(' ')[0]}`}
        />
      )}
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mb-5 flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-neutral-text-muted">
                {description}
              </p>
            </div>
          </div>
          {badge && (
            <span
              className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${badgeBg} ${badgeText}`}
            >
              {badge}
            </span>
          )}
        ...
        <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {infoItems.map((item, idx) => (
            ...
            >
              <p className="text-xs font-medium text-neutral-text-muted">
                {item.label}
              </p>
              <p className="mt-0.5 text-sm font-semibold text-neutral-text-primary">
                {item.value}
              </p>
            </div>
          ))}
        ...
        {/* Last result */}
        {lastResult && <div className="mb-5">{lastResult}</div>}
...
function ResultBanner({ variant, items }: ResultBannerProps) {
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-4 rounded-xl border p-3.5 ${colors}`}
    >
      <svg
    ...
    <>
      ...
      <div className="mb-6 rounded-2xl border border-brand-accent-light bg-brand-accent-light/30 p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-brand-accent-dark">
              Seed Credentials
            </p>
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              {[
                ...
                >
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-semibold ${account.color}`}
                  >
                    {account.role}
                  </span>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          icon={
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7M4 7h16M4 7l2-3h12l2 3M9 11v6m6-6v6"
              />
            </svg>
          }
                ...
                items={[
                  {
                    ...
                    value: seedResult.ticketsCreated,
                  },
        ...
        <AdminCard
          icon={
            <svg
              className="size-6 text-ui-danger"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1.75}
                ...
                items={[
                  {
                    ...
                    value: sweepResult.ticketsDeleted,
                  },
